...
function openDialog(url, _id, _title, _width, _height, left, top) {
    top = 0;
    var _href = window.location.pathname.substr(1);
    if (_href.indexOf('MainForm.aspx') == -1) {//兼容不在框架中打开子页面的方法
        var arr = getUrlPathName();
        var arr1 = getUrlPathName(url);
        if (arr.length > 0 && arr1.length > 0) {
            if (arr[0] == arr1[0]) {
                url = url.replace("../", "");
                url = url.replace(arr1[0], "");
                if (url.substr(0, 1) == "/")
                    url = url.substr(1);
            }
            if (arr[1] == arr1[1]) {
                url = url.replace("../", "");
                url = url.replace(arr1[1], "");
                if (url.substr(0, 1) == "/")
                    url = url.substr(1);
            }
            if (arr[2] == arr1[2]) {
                url = url.replace("../", "");
                url = url.replace(arr1[2], "");
                if (url.substr(0, 1) == "/")
                    url = url.substr(1);
            }
        }
...
function getUrlPathName(url) {
    var _href = "";
    if (url == undefined)
        _href = window.location.pathname.substr(1);
    else {
        url = url.replace("../", "");
        _href = url;
    }
    _href = _href.substr(0, _href.lastIndexOf('/'));
    return _href.split('/')
}
/*弹出网页
/*url:          表示请求路径
...
**/
var GetRowIndex = -1; //索引号 多选时只保留最后一个
var GetRowIndexArr = [];//选中行的索引列表
function PQLoadGrid(obj_ID, url, colM, sort, PageSize, topVisible) {
    ...
    $(obj_ID).pqGrid({
        ...
        rowSelect: function (evt, ui) {
            ...
            GetRowIndexArr.push(ui.rowIndxPage);
        },
        rowUnSelect: function (evt, ui) {
            removeByValue(GetRowIndexArr, ui.rowIndxPage);
        }
    });
...
obj：ID
**/
function pqGridAllRows(obj) {
    return $(obj).pqGrid('getData');
}
/**选中某行
obj：ID，index：行索引号 不传入index时取消所有选中
**/
function pqGridSetSelection(obj, index) {
    if (index != undefined) {
        return $(obj).pqGrid('setSelection', { rowIndx: index });
    }
    else {
        return $(obj).pqGrid('setSelection');
    }
}
/**选中所有行
obj：ID
**/
function pqGridSetSelectionTotal(obj) {
    var rows = pqGridAllRows(obj).length;
    for (var index = 0; index < rows; index++) {
        $(obj).pqGrid('setSelection', { rowIndx: index });
    }
    return true;
}
/**获取表格列值(选中行)
obj_ID：ID，rowCode：列号
**/
...
/**获取表格列值Array(选中行)
obj_ID：ID，rowCode：列号 当rowCode不传入时返回选中行索引数组
**/
function GetPqGridRowArray(obj_ID, rowCode) {
    if (GetRowIndexArr.length > 0) {
        if (rowCode == undefined) {
            return GetRowIndexArr;
        }
        var DM = $(obj_ID).pqGrid("option", "dataModel");
        var data = DM.data;
        var arr = new Array();
        for (var i = 0; i < GetRowIndexArr.length; i++) {
            arr.push(data[GetRowIndexArr[i]][rowCode]);
        }
        return arr;
    }
    else {
        showTopMsg("你没有选择任何行", 4000, 'warning');
        return new Array();
    }
}
/**获取表格列值
obj_ID：ID，rowCode：列号
**/
function GetPqGridPeriodArray(obj_ID, rowCode) {
    var data = $(obj_ID).pqGrid("option", "dataModel").data;
    var count = $(obj_ID).pqGrid("option", "dataModel").data.length;
    var arr = new Array();
    for (var i = 0; i < count; i++) {
        arr.push(data[i][rowCode]);
    }
    return arr;
}
/**删除array中指定元素
arr：array对象，val：删除的元素
**/
